...
const StatusIndicator = ({ status, isEditing, newStatus, onStatusChange }) => {
  return isEditing ? (
    <select
      ...
      className="bg-gray-800/50 text-sm px-2 py-1 rounded focus:outline-none focus:
    ring-1 focus:ring-orange-500"
    >
...
const BookingCardActions = ({
  ...
  return (
    <div className={BookingPageStyles.bookingActions}>
      <div
        className="items-center text-orange-400 hidden md:flex"
...
const BookingCardDetails = ({ booking }) => {
  const address = booking.address || {}; // safeguard
  ...
  return (
    <div className={BookingPageStyles.bookingDetails}>
      <div className={BookingPageStyles.bookingDetailsGrid}>
        ...
        >
          <Detail icon={<FaMapMarkerAlt />} label="Street" value={address.street || "N/A"} />
          <Detail icon={<FaCity />} label="City" value={address.city || "N/A"} />
          <Detail icon={<FaGlobeAsia />} label="State" value={address.state || "N/A"} />
          <Detail icon={<FaMapPin />} label="ZIP Code" value={address.zipCode || "N/A"} />
        </Panel>
...
const NoBookinsView = ({ onResetFilters }) => (
...
const PageHeader = () => (
  <div className={BookingPageStyles.headerContainer}>
    ...
    <p className={BookingPageStyles.subtitle}>
      Manage all bookings with detailed information ans status updates.
    </p>
  </div>
);
...
const Booking = () => {
  ...
  try {
    ...
    // Make sure we correctly extract the data array
    const raw = Array.isArray(res.data)
      ? res.data
      : res.data.data || res.data.bookings || [];

    // ✅ Fixed mapping with proper customer details extraction
    const mapped = raw.map((b, i) => {
      ...
      // Safely extract customer info (handles both object and flat)
      const customerObj = b.customer || b.user || {};
      const customerName =
        customerObj.name ||
        ...
        b.customer ||
        "";
      const email = customerObj.email || b.email || "";
      const phone = customerObj.phone || b.phone || "";
  ...
  try {
    ...
    await api.patch(`/api/bookings/${currentBooking._id}/status`, {
      status: newStatus,
    });
    ...
    <div className={BookingPageStyles.pageContainer}>
      ...
      <div className="space-y-4">
        ...
        {filterBookings.length === 0 && (
          <NoBookinsView onResetFilters={handleResetFilters} />
        )}
      </div>
    </div>
  );
